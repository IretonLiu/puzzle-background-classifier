...
def read_data():
    ...
    images = np.array([cv2.cvtColor(cv2.imread(image), cv2.COLOR_BGR2RGB)
...
def to_feature_vector(images, feature_type):
    ...
    if feature_type == 'rgb':
        ...
        hsv = np.array([cv2.cvtColor(image, cv2.COLOR_RGB2HSV)
                       for image in images]).reshape(-1, 3)
        ...
        dog = np.array([cv2.GaussianBlur(image, (3, 3), 0) -
                       cv2.GaussianBlur(image, (5, 5), 0) for image in float_images])
        ...
        xy = np.array([np.indices(image_dim[:2]).transpose((1, 2, 0))
                      for image in images]).reshape(-1, 2)
        # normalize the hsv values
        return np.hstack((hsv.astype(np.float32) / np.array([179, 255, 255]), xy.astype(np.float32) / np.array(image_dim[:2])))
        ...
        for feature in features:
            ...
            for background_h in background_h_list:
                ...
                try:
                    ...
                    print("Failed to fit background GMM with h = {}".format(
                        background_h))
                    continue
